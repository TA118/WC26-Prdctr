...
export function MyGroupsPage() {
  ...
  const fetchGroups = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('group_members')
      .select('group_id, prediction_groups(id, name)')
      .eq('user_id', user.id);

    if (!data) { setLoadingGroups(false); return; }

    // For each group, fetch all members and their predictions to compute rank
    const groups: Group[] = await Promise.all(
      data.map(async (row: any) => {
        const gid = row.prediction_groups.id;
        ...
        // Points are 0 until actual results exist — sorted by points desc
        ...
        return {
          id: gid,
          name: row.prediction_groups.name,
          userRank: rank || 1,
          userPoints: myPts,
    ...
    // Compute global rank
    const { data: allPreds } = await supabase.from('full_predictions').select('user_id');
    const totalUsers = allPreds?.length ?? 0;
    // All points are 0 until actual results exist, so rank = position among all users
    const myPos = allPreds?.findIndex((p: any) => p.user_id === user.id) ?? -1;
    setGlobalRank(myPos >= 0 ? myPos + 1 : totalUsers);
  ...
  return (
    <div className="home-page">
      ...
      <div className="home-hero">
        <h1 className="home-title">👥 My Groups</h1>
      ...
      <div className="groups-list">
        {!user ? (
          ...
          myGroups.map(g => (
            <button key={g.id} className="group-card" onClick={() => navigate(`/prediction/full/groups/${g.id}`)}>
        ...
        <button className="group-card group-card--global" onClick={() => navigate('/prediction/full/groups/global')}>
...
function CreateJoinModal({
  ...
  async function handleCreate() {
    ...
    const { data: nameCheck } = await supabase.from('prediction_groups').select('id').eq('name', name).maybeSingle();
    const { data: passCheck } = await supabase.from('prediction_groups').select('id').eq('invite_password', pass).maybeSingle();
